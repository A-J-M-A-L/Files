...
exports.smsg = (conn, m, hasParent) => {
     ...
     */
     m.reply = async (text, chatId, options) => {
    	let pp = await conn.profilePictureUrl(m.sender, 'image').catch(_ => 'https://telegra.ph/file/7d9d70dddf6cdef3dd6b3.jpg')
        let { data } = await conn.getFile(await(await require('node-fetch')(pp)).buffer())
        conn.reply(chatId ? chatId : m.chat, text, m, { contextInfo: { mentionedJid: conn.parseMention(text), externalAdReply: { title: 'ᴡɪᴢᴀʀᴅ ᴍᴜʟᴛɪ ᴅᴇᴠɪᴄᴇ ʙʏ ᴀᴊᴍᴀʟ', body: wm, sourceUrl: 'https://youtube.com/channel/UCLegt7MKqNBxJjIkE_QNPdA', thumbnail: data }}, options })
    }
